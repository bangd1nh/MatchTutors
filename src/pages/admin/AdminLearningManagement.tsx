...
const AdminLearningManagement = () => {
   ...
   const resolvedLogs: AdminResolvedCaseLog[] =
      resolvedCommitments?.data ?? [];
            ...
            <TableBody>
               {pendingCommitments.map((item) => (
                  <TableRow key={item._id}>
                     <TableCell>
                        <p className="font-medium">{getPersonName(item.student)}</p>
                        <p className="text-xs text-muted-foreground">
                           {getPersonEmail(item.student)}
                        </p>
                     </TableCell>
                     <TableCell>
                        <p className="font-medium">{getPersonName(item.tutor)}</p>
                        <p className="text-xs text-muted-foreground">
                           {getPersonEmail(item.tutor)}
                        </p>
                     </TableCell>
                     <TableCell>{item.teachingRequest?.subject ?? "N/A"}</TableCell>
                     <TableCell>
                        <div className="space-y-1 text-xs">
                           <Badge variant="secondary" className="capitalize">
                              HS: {item.cancellationDecision?.student.status ?? "N/A"}
                           </Badge>
                           <Badge variant="outline" className="capitalize">
                              GS: {item.cancellationDecision?.tutor.status ?? "N/A"}
                           </Badge>
                        </div>
                     </TableCell>
                     <TableCell className="max-w-xs text-sm">
                        {item.cancellationDecision?.reason ?? "Chưa cung cấp"}
                     </TableCell>
                     <TableCell>
                        <div className="text-xs leading-5">
                           <p>Tổng: {item.absenceStats?.totalSessions ?? 0}</p>
                           <p className="text-red-600">
                              Học viên: {item.absenceStats?.studentAbsent ?? 0}
                           </p>
                     ...
                     <TableCell>
                        <Button
                           variant="outline"
                           size="sm"
                           onClick={() => navigate(`/admin/learning/${item._id}`)}
                        >
                           Xử lý
                        </Button>
                     </TableCell>
         ...
         <Table>
            ...
            <TableBody>
               {resolvedLogs.map((item) => (
                  <TableRow key={item.logId ?? `${item.commitmentId}-${item.handledAt}`}>
                     <TableCell>
                        <p className="font-medium">{getPersonName(item.student)}</p>
                        <p className="text-xs text-muted-foreground">
                           {getPersonEmail(item.student)}
                     ...
                     <TableCell>
                        <Button
                           ...
                           onClick={() =>
                              item.commitmentId &&
                              navigate(`/admin/learning/${item.commitmentId}`, {
                                 state: { viewMode: "history", log: item },
                              })
                           }
                        >
                           Xem chi tiết
   ...
   return (
      <Card>
         <CardHeader>
            ...
            <CardDescription>
               Theo dõi các yêu cầu hủy đang chờ xử lý và lịch sử các case đã giải quyết.
            </CardDescription>
         </CardHeader>
         <CardContent>
            <Tabs
            ...
            >
               <TabsList className="grid w-full grid-cols-2">
                  ...
                  <TabsTrigger value="history">Lịch sử ({historyTotal})</TabsTrigger>
               </TabsList>
